...
- Strony stałe: `src/strony/*.md` (układ `strona.njk`, adres `/nazwa-pliku/`) — m.in. `/o-danych/` (polityka prywatności, edytowalna w panelu). Czcionka Lora hostowana lokalnie (`@fontsource/lora` → `/fonts/`, `src/css/fonty.css`) — strona nie łączy się z Google w przeglądarce odwiedzającego; nie dodawać zewnętrznych czcionek/skryptów/analityki bez aktualizacji `/o-danych/`.
- Formularz kontaktowy: Web3Forms — działa, potwierdzone 26.09 (pola name/email/message; klucz w `src/_data/formularz.json`, publiczny z założenia; wiadomości idą na e-mail przypisany do klucza). Netlify Forms wyłączone (płatne). Web3Forms = podmiot przetwarzający dane → do polityki prywatności.
