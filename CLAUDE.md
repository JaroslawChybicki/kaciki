...
- Terminy: funkcja `netlify/functions/terminy.mts` → `/api/terminy?od=- Terminy: funkcja `netlify/functions/terminy.mts` → `/api/terminy`, Google Calendar API; zmienne `GOOGLE_CALENDAR_ID`, `GOOGLE_API_KEY` w Netlify.do=- Terminy: funkcja `netlify/functions/terminy.mts` → `/api/terminy`, Google Calendar API; zmienne `GOOGLE_CALENDAR_ID`, `GOOGLE_API_KEY` w Netlify.max=`, Google Calendar API (kalendarz zenkaciki@gmail.com); zmienne `GOOGLE_CALENDAR_ID`, `GOOGLE_API_KEY` w Netlify. Strona `/terminy/` (`src/js/kalendarz.js`): widoki Lista / Miesiąc / Rok; na stronie głównej 5 najbliższych. Wspólne formatowanie dat: `window.KacikiTerminy` w `main.js`.
